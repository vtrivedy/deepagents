...
class DeepAgent:
    ...
    def __init__(self, graph: CompiledStateGraph, sandbox: Any | None = None):
        self.graph = graph
    ...
    def cleanup(self):
        ...
        if self._sandbox:
            # Handle async cleanup (sandbox providers have async cleanup)
            import asyncio
            import inspect

            if hasattr(self._sandbox, 'cleanup') and callable(self._sandbox.cleanup):
                cleanup_method = self._sandbox.cleanup
                if inspect.iscoroutinefunction(cleanup_method):
                    # Async cleanup - need to run in event loop
                    try:
                        loop = asyncio.get_event_loop()
                        if loop.is_running():
                            # If loop is already running, schedule the cleanup
...
def create_deep_agent(
    ...
    *,
    ...
    if sandbox is not None:
        ...
        sandbox_backend = provider  # Provider now implements BackendProtocol directly!
        backend = lambda rt: CompositeBackend(
            default=StateBackend(rt),
            routes={"/workspace/": sandbox_backend}
        )
        # Bash tool will be auto-generated by FilesystemMiddleware
    if model is None:
